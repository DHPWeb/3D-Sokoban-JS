	...
	for(var i = 0; i < 16; i++)
	{
		for(var j = 0; j < 20;j++)
		{
			...
			else if(car == '#')
			{
				...
				env.setShapeTexture(index, "Textures/uptron.jpg", 1.0);
			...
			{
				env.shapes.push(BABYLON.Mesh.CreatePlane("Plane",1.0,env.scene));
				env.setShapeTexture(index, "Textures/target1.jpg", 1.0);
				env.setShapePosition(index, i, -0.5, j);
				env.shapes[index].rotation.x = 1.5707963267949;
				index++;					
			}
...
function chargeTab(no){
	...
	xhr.open("GET","Tableaux/php1.php", false);
	xhr.send(null);
